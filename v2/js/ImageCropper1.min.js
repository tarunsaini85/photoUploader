...
var initSmallImage = function(newCoordinates){
		/*if(document.querySelectorAll){
            var crop_dottedDiv = document.querySelectorAll('.crop_dottedDiv')[0];
        }
        else{
            var divs = document.getElementById('photoUploadCropArea').getElementsByTagName('div');
            for(var i=0; i<divs.length; i++){
                if(divs[i].className == 'crop_dottedDiv'){
                    var crop_dottedDiv = divs[i];
                    break;
                }
            }
        }*/
		var crop_dottedDiv=cropDiv_dotted;
		//console.log(crop_dottedDiv,cropDiv_dotted)
		var photoFrameObj = thumbImages;
		//console.log(photoFrameObj)		
		for(var i = 0 ; i<photoFrameObj.length; i++){			
		var crop_dotted = (crop_dottedDiv.style.width).split('px')[0], 
			crop_dotted_height = (crop_dottedDiv.style.height).split('px')[0] *  arrGlobalConfigParams.crop_script_fixedRatio;			
			photoFrame = (photoFrameObj[i].parentNode.style.width).split('px')[0],
			photoFrame_height = (photoFrameObj[i].parentNode.style.height).split('px')[0] *  arrGlobalConfigParams.crop_script_fixedRatio,
			diff = crop_dotted - photoFrame,
			diff_height = crop_dotted_height - photoFrame_height,
			actual_pixel = actual_width * (diff/crop_dotted),
			actual_pixel_height = actual_height * (diff/crop_dotted_height),
			actual_top = parseInt(newCoordinates.top) * ((parseInt(actual_height) - parseInt(actual_pixel_height))/parseInt(actual_height)),
			actual_left = newCoordinates.left * ((actual_width - actual_pixel)/actual_width);
    	photoFrameObj[i].style.marginLeft = -actual_left+"px";
		photoFrameObj[i].style.marginTop = -actual_top+"px";
		photoFrameObj[i].style.width = actual_width - actual_pixel + 'px';
		}
}
...
var cropScript_setCropSizeByInput = function()
{
	...
	cropDiv_dotted.style.left = (crop_imageWidth/2)-(parseInt(cropDiv_dotted.style.width)/2)+ 'px';
    //initSmallImage({top:heightMargin, left:widthMargin});
	initSmallImage({top:heightMargin, left:(crop_imageWidth/2)-(parseInt(cropDiv_dotted.style.width)/2)});
	repositionSmallSquares();		
	resizeTransparentSquares();
}
...
this.setAction = function(func1, func2, OkButton, cancelButton, captFlg){
	...
	okButtonId.onclick = callbackOk;
    function callbackOk(){
        //func1.call(window, x1, y1, x2, y2);
        func1.call(window, window.x1_crp, window.y1_crp, window.x2_crp, window.y2_crp, captFlg);
        return true;
    }
	...
	cancelButtonId.onclick = callbackCancel;
    function callbackCancel(){
        func2.call(window);
        return true;
    }
}
this.init_imageCrop = function(imageId, imgContainer,smallImgClass, func1, func2, OkButton, cancelButton, smallSquareImg, arrayConfig, captFlg)
{
	thumbImages=smallImgClass;
	smallSquareImage = smallSquareImg;
	arrayConfig = arrayConfig || {};
	imageContainer = imgContainer;
	configureGlobalParameters(arrayConfig);
	crop_imageWidth = imageId.width;
	crop_imageHeight = imageId.height;
